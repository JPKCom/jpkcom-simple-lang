    ...
    public function verify_download_checksum( $reply, string $package, \WP_Upgrader $upgrader ) {
        ...
        if ( ! $remote ) {
            return new \WP_Error(
                ...
                __( 'Security verification failed: the update manifest could not be loaded, so the package cannot be verified.', 'jpkcom-simple-lang' )
            );
        }
        ...
        if ( empty( $remote->checksum_sha256 ) ) {
            return new \WP_Error(
                'checksum_missing',
                __( 'Security verification failed: the update manifest contains no SHA-256 checksum, so the package cannot be verified.', 'jpkcom-simple-lang' )
            );
        }
